...
- ✅ **Sección de opinión del alumno agregada en modulo6.html** (ver nota técnica 14): estrellas + comentario libre, se guarda en Firestore (colección `opiniones`), no depende de Blaze ni Stripe. Bug de UI encontrado y corregido en la misma prueba: sin Firestore creado todavía en el proyecto, el envío se quedaba colgado para siempre — se agregó timeout de 8s. Pendiente para el usuario: crear la base de datos Firestore y desplegar `firestore.rules` actualizado
