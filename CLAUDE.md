...
- [ ] **⚠️ Despliegue bloqueado (sesión 7):** el pipeline de GitHub Pages (`pages build and deployment`) está fallando con `Error: Deployment failed, try again later.` / `Timeout reached, aborting!` — confirmado que es un problema de infraestructura de GitHub (no del código ni la configuración del repo). El `build` completa bien (25s), pero el `deploy` se queda atorado en `deployment_queued` y aborta. El usuario ya intentó: re-run del job, cambiar Source de Pages entre "GitHub Actions" y "Deploy from a branch" (ida y vuelta) sin éxito aún. Los 4 archivos con el fix de navegación (modulo2-5.html) están confirmados correctos tanto en el repo como probados en local, solo falta que el deploy a Pages se destrabe. **Seguimiento: pedir al usuario que revise si ya se desplegó en su próxima sesión**
...
- ⚠️ Pendiente — Revisar en la próxima sesión si el deployment de GitHub Pages ya se destrabó y el sitio en vivo refleja los botones de navegación
